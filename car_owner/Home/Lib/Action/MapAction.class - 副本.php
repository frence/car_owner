    ...
    public function portinfolist() {
        ...
        if ($parkType==1 || $parkType==2) {
            if ($parkType == 1) {
                ...
                $where = "  port_type_id <> 10    and Status >=0   and  AreaNo in (select AreaNo from car_area where AreaName like '%" . $city . "%') and  service_module_id =1 ";
            } else {
                 
                //$where = "AreaNo in (select AreaNo from car_area where AreaName like '%".$city."%')  and ServiceType = 0 and port_type_id=1 and Status >=0";
                $where = " port_type_id <> 10 and  Status >=0 and  AreaNo in (select AreaNo from car_area where AreaName like '%" . $city . "%') and  service_module_id <>1 ";
