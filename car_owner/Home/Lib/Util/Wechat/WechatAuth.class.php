...
class WechatAuth {
	...
	private $UserApiURL = 'https://api.weixin.qq.com/datacube';
	/**
	 * $url_1 = urlencode($this->api_site_url('login/'.$api_id));
			$url_2 = urlencode($this->api_site_url('index'));
			$url = $this->{$this->api}->get_connect_oauth2_url($url_1, $url_2);
			redirect($url);
	...
	public function __construct($appid, $secret, $token = null) {
		if ($appid && $secret) {
			...
			throw new Exception('参数错误！appid:'.$appid.';secret:'.$secret);
		}
	}
	...
	public function messageCustomSend($openid, $content, $type = self::MSG_TYPE_TEXT) {
		...
		// 根据类型附加额外数据
		$data[$type] = call_user_func(array (
		self,
		$type
		) , $content);
	...
	protected function api($name, $data = '', $method = 'POST', $param = '') {
		...
		if (!empty($data)) {
			...
			create_function('&$value','$value = urlencode($value);'));
	...
	public function getUserData($type, $data = array()) {
		$params = array (
		'access_token' => $this->accessToken
		);
		...
		$data = self::http($this->UserApiURL . '/' . $type , $params , $data , 'POST');
